...
**4.** The main methodological consequence is upstream of any particular ecological model: preserve observation-process distinctions before fitting the downstream model, and calibrate operational decisions against explicit error criteria rather than inherited score values. TNOA does not claim to introduce abstention, imperfect-detection correction or observation-process modelling individually, and the current quantitative results are closed-world rather than field-calibrated. Field evidence adapters and numerical thresholds must be revalidated for each sensing system.
