...
**Figure 2. A raw nuisance-score threshold did not retain its operating meaning after the score representation changed.** (a) Nuisance ranking against both registered negative families remained strong while recall at the inherited threshold 0.55 fell to 0.23125. (b) Pass rates of spatial, temporal and combined nuisance components across registered Pi5 values at the inherited 0.55 boundary show that the numerical threshold no longer represented a stable process boundary. (c) A pooled false-attribution calibration exceeded the prespecified alpha=0.05 criterion for the coupled negative family, whereas family-wise calibration reduced held-out false nuisance attribution to approximately 0.04444 while retaining nuisance coverage. All values are closed-world validation results, not field false-positive rates. <!-- C6 C7 -->
...
def main() -> None:
    ...
    callouts = {
        ...
        "### 3.2 Binary coarsening discarded information about synthetic target prevalence": "### 3.2 Binary coarsening discarded information about synthetic target prevalence\n\n*See Figure 3a–b.*",
