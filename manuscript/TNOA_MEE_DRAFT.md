...
Accordingly, we evaluate TNOA as a tested method rather than as a conceptual workflow. One operational experiment asks whether a nuisance decision can retain a declared error meaning after its numerical score representation changes. A second asks how much downstream information about known ecological estimands is lost when B/T/N/U is collapsed to target/not-target. Post-freeze controls then ask how prevalence weighting and additional observation columns alter the magnitude without treating every refinement as semantic evidence. Falsified predictions constrain, rather than expand, those methodological claims.

### 1.4 Questions

We evaluate four questions. First, can an operational nuisance decision remain meaningful when the numerical representation of nuisance changes, or must the decision boundary be tied to a prespecified error criterion rather than an inherited raw threshold? Second, how much information about downstream ecological estimands is lost when distinct B/T/N/U observation-process states are collapsed to target/not-target? Third, does the preregistered prediction of a narrow ambiguity ridge at matched target and nuisance timescales survive a frozen synthetic test? Fourth, which information-preservation and unresolved-observation results survive explicit prevalence, weighting and structural-specificity controls?
...
The final prior-art audit motivated an additional deterministic refinement after the frozen science and original D1 analysis were complete. Using the same frozen rows and 3,003 simplex mixtures, we compared four nested observation vocabularies: TARGET/not-TARGET; TARGET/NUISANCE/other; B/T/N/U with U reasons collapsed; and B/T/N/U with the two frozen V14b U-reason buckets separated. Because this question was formulated after inspecting earlier results, it is explicitly **post-freeze and not preregistered**. No new synthetic worlds were generated and no observer or threshold was changed. <!-- D3 -->

We evaluated five latent estimands: target prevalence, nuisance prevalence, target+nuisance co-occurrence, coupled-response prevalence and any-deviation prevalence. We repeated the final two-way refinement across all 34 registered single-axis slices. D3 records how much identification narrows as an additional non-redundant observation column is retained; it does not by itself establish that the narrowing is caused by the semantic meaning of the reason labels. <!-- D3 -->
...
We first stratified the same 3,003 compositions by their known target prevalence `theta` in increments of 0.1 and recomputed median identification widths. We also summarized rare-target subsets `theta<=0.1`, `theta<=0.2` and `theta<=0.3`. Second, we placed a bounded density-ratio class directly on the 3,003 composition weights: relative to uniform composition weighting, each multiplier `r_i` satisfied `1/kappa <= r_i <= kappa` and mean `r_i=1`. We use this audit primarily to stress-test the D1 B/T/N/U-versus-binary comparison. These are worst-case weighted-mean ratios, not weighted medians and not ecological priors. Full definitions and values are in `docs/PREVALENCE_WEIGHTING_SENSITIVITY.md`. <!-- D4 -->

### 2.11 Post-freeze random-split control for D3 semantic specificity

A subsequent reviewer-motivated control tested whether D3's additional narrowing was specific to the meanings of the two frozen U-reason buckets. This D5 analysis is **post-freeze and not preregistered**. It leaves the generic B/T/N/U emission matrix fixed and changes only how its U column is represented; no new worlds, observers, thresholds or latent regimes are introduced. <!-- D5 -->

We used three controls. A constant 50:50 U split adds a redundant column and therefore no new regime discrimination. For 500 unlabeled two-way controls, each of the six latent regimes independently received a random `p~Uniform(0,1)` and its U emission was split into `pU` and `(1-p)U`. For 500 unlabeled three-way controls, each regime's U emission was split according to `Dirichlet(1,1,1)`. Random seed `0` is fixed for reproducibility; these controls are not ecological alternative reason systems and are not used as a Monte Carlo significance test. We report the distribution of identification widths and the rank/null-space dimension of the resulting six-regime constraint system. <!-- D5 -->

### 2.12 Structural interpretation audit
...
The post-freeze vocabulary ablation produced an additional numerical narrowing when the two frozen V14b U-reason buckets were separated: target-prevalence median width changed from `0.02992` with generic U to `0.00408`, and T+N co-occurrence width from `0.10494` to `0.01484`. <!-- D3 --> However, D5 shows that this additional gain is **not specific to the reason semantics**. A constant 50:50 split left target-prevalence width unchanged at `0.0299207`, whereas the median across 500 unlabeled regime-dependent two-way splits was `0.0050075`; **48.0%** of those arbitrary splits were equal to or narrower than the frozen semantic split. <!-- D5 --> Across all five estimands, the random-equal-or-better fraction ranged from `0.480` to `0.672`. All 500 unlabeled three-way U splits produced a full-rank six-regime constraint system and point identification to numerical tolerance for all five estimands. <!-- D5 --> Thus D3 demonstrates the value of additional non-redundant regime-discriminating observation structure in this six-regime design, but it does not isolate an information advantage caused by the selected U-reason meanings.

The composition lattice is concentrated toward higher target prevalence: only `141/3003` compositions (`4.70%`) have known target prevalence `<=0.2`. <!-- D4 --> The primary D1 information-preservation gain did not disappear in this rare-target subset: median target-prevalence width was `0.07410` after target/not-target collapse versus `0.000175` with B/T/N/U retained. <!-- D4 --> When the 3,003 composition weights were themselves allowed to vary adversarially within the bounded density-ratio class, the `kappa=10` worst case still removed at least `57.5%` of binary weighted-mean width by retaining B/T/N/U. <!-- D4 --> These D4 results are reviewer-motivated post-freeze sensitivity analyses, not ecological prevalence priors or preregistered predictions.
...
The empirical information-preservation result therefore rests on retaining distinct B/T/N/U observation-process states before binary collapse. Finer subdivision can narrow compatible sets further when it adds non-redundant regime information, but the current experiment does not attribute that extra narrowing to reason semantics.
...
The direction of the information comparison is structural, not a discovered performance advantage: a deterministic coarsening is a garbling of the richer statistical experiment [@blackwell1953comparison]. What the known-truth experiment contributes is the scale and conditions of the consequence in the registered observation process. Median compatible target-prevalence width increased from about `0.030` with B/T/N/U retained to `0.266` after binary coarsening, and the B/T/N/U advantage remained large in the rare-target subset and under bounded composition reweighting. <!-- D1 D4 --> This is the primary information-preservation result.

The D3/D5 pair provides a different lesson. The frozen two-way U split narrowed target-prevalence width to `0.00408`, but arbitrary unlabeled regime-dependent splits commonly did the same: 48% of 500 two-way random controls matched or exceeded the frozen semantic split for target prevalence. <!-- D3 D5 --> The added narrowing therefore cannot be attributed to the meanings `no-supported-evidence` versus `overlap-or-attribution` from this experiment. It is substantially explained by the generic rank/identifiability gain of adding a non-redundant observation column; the exact width also depends on the orientation of that column relative to the estimand. This self-control narrows, rather than weakens, the main claim: preserving distinct observation-process states matters, while not every additional category should be interpreted as semantic evidence.

The comparison with prior ecological methods is therefore specific. Continuous-score occupancy already shows that thresholding classifier scores is not necessary [@rhinehart2022continuous], and multievent or partial-observation models already preserve uncertain ecological events [@pradel2005multievent; @campbellgrant2023partial]. TNOA's contribution is to define a process-semantic **upstream record**—including nuisance, observability and attribution distinctions—and to quantify how much downstream identifying information is lost when the core B/T/N/U record is collapsed before ecological modelling.
...
### 4.3 Unresolved reasons remain design semantics, not a demonstrated information premium

An unresolved observation is not a single type of measurement problem. No-supported-evidence, target+nuisance overlap, missing attribution and insufficient observability can imply different follow-up actions: more sampling may help evidence shortage, whereas overlap or attribution failure may require an independent reference channel. That is the architectural reason to retain provenance.

The frozen V14b surface, however, contains only two aggregated U-reason buckets: `information_absent` (reported here as no-supported-evidence) and `overlap_or_attribution`. The reusable API later exposes four U reasons and should not be read as a one-to-one empirical decomposition of those frozen columns. The D5 control further shows that D3's partial-identification improvement cannot validate the meanings of the two frozen reason labels, because arbitrary regime-discriminating splits provide comparable narrowing. <!-- D5 --> Reason provenance may therefore be scientifically useful for measurement planning, but its semantic value requires independent measurement justification rather than being inferred from D3's width reduction.

The `Pi1` decomposition still provides a descriptive process illustration: after `Pi1=1`, the frozen no-support bucket declined while the combined overlap/attribution bucket continued to rise. <!-- C11 D2 --> This supports reason-aware reporting within the frozen semantics, not a universal claim that those two categories are the uniquely informative partition of U.
...
The D5 specificity control adds a third corrective result: an initially attractive semantic interpretation of D3 did not survive an unlabeled rank-matched control. We therefore retain the D3 numerical narrowing but retire the claim that its magnitude demonstrates a semantic-specific information advantage. <!-- D5 -->
...
A camera-trap application might define T as positive focal-species evidence, N as positive evidence for motion or visibility processes that mimic or mask detection, and O as adequate camera geometry and image support. A passive-acoustic application could define T as focal-call support, N as masking or overlapping non-target sound and O as sufficient microphone/temporal support. An interaction camera can additionally retain a local target-coupled response as C until independent attribution supports a target link. The raw features and calibration rules change across systems, but the fail-closed sequence does not. Each application must validate its own evidence adapters, reason semantics and error criteria.
...
D3, D4 and D5 were motivated after inspection of the frozen science and original D1 analysis. They are therefore explicitly post-freeze and not preregistered. Registered-axis slices and bounded composition reweighting constrain design dependence but do not establish robustness to arbitrary ecological prevalence weighting. D5 shows that the additional D3 narrowing is not semantic-specific: arbitrary non-collinear U splits often give comparable or greater narrowing because the six-regime compatible-mixture null space shrinks as independent observation columns are added. The six-regime/five-state geometry is therefore part of the D3 effect size. The study does not test how that magnitude would change with a larger or different latent-regime set.

The frozen V14b result stores only two U-reason columns, whereas the reusable API distinguishes four unresolved reasons. The frozen `overlap_or_attribution` source bucket combines at least simultaneous T+N support and unresolved indirect-only attribution in the V14b decision code, while insufficient observability has no separate frozen D3 column. Consequently, the API reason vocabulary is an implementation refinement, not a four-way empirical decomposition validated by the frozen phase surface.

All D1/D3/D4/D5 information comparisons also condition on a frozen, effectively known emission map. This isolates information lost through observation coarsening, but it does **not** account for the greater annotation and calibration burden that a richer observation vocabulary may require under a fixed validation budget. With finite calibration data, uncertainty in a richer emission map could offset part of the identification gain reported here. We therefore do not claim greater information per annotation, per unit cost or per field hour. A finite-budget comparison would require a new measurement-design specification for annotation allocation, grouped dependence, rare-state sampling and emission-matrix uncertainty.
...
Automated ecological sensors should not be required to manufacture biological absence from unresolved observations. TNOA separates positive target support, positive nuisance support, measurement support and attribution before an observation is passed downstream. In the frozen synthetic study, the strongest evidence was that an inherited raw threshold lost its operating meaning after a representation change, that a predeclared family-conditional false-attribution criterion restored the declared held-out control, and that collapsing B/T/N/U to target/not-target substantially widened compatible sets for known downstream ecological estimands across the registered simplex and explicit prevalence/weighting stress tests. <!-- C6 C7 D1 D4 -->

The post-freeze D3/D5 controls add an important qualification: further subdivision of U can narrow compatible sets when it adds non-redundant regime information, but the present experiment does not show that the size of that gain is specific to the selected reason semantics. <!-- D3 D5 --> The practical recommendation is therefore to calibrate observation decisions against explicit error criteria, preserve the core observation-process distinctions before binary collapse, and retain finer reason provenance when its measurement semantics are independently justified rather than treating extra categories themselves as evidence.

---

## Data and code availability

The repository contains the manuscript-facing provenance package, immutable result identifiers, post-freeze derived analyses, figure scripts, claim guards and a minimal reusable Python implementation. The reusable API and CSV CLI are documented in `docs/REUSABLE_IMPLEMENTATION.md` and demonstrated with `examples/minimal_evidence.csv`; the fail-closed sensor-to-field calibration sequence is documented in `docs/FIELD_TRANSLATION_PATHWAY.md`; the post-freeze vocabulary ablation is documented in `docs/OBSERVATION_VOCABULARY_ABLATION.md`; the prevalence/composition-weight sensitivity is documented in `docs/PREVALENCE_WEIGHTING_SENSITIVITY.md`; and the semantic-specificity control is documented in `docs/REASON_SPLIT_SPECIFICITY_CONTROL.md`.

Historical one-shot results remain tied to their original workflow runs and artifact hashes; later software checks do not replace the frozen scientific record.
